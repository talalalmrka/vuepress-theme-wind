...
export const useActiveHeaderLinks = (options: ActiveHeaderLinksPluginOptions = {}): void => {
  // if (__VUEPRESS_SSR__) return

  const router = useRouter()
  const { theme } = useData()
  const themeActiveHeaderOptions = (theme.value.pluginOptions
  ...
  options = {
    ...themeActiveHeaderOptions,
    ...options
  }
  const { headerLinkSelector, headerAnchorSelector, delay, offset } = options
  const setActiveRouteHash = (): void => {
    ...
    for (let i = 0; i < existedHeaderAnchors.length; i++) {
      ...
      const nextAnchor = existedHeaderAnchors[i + 1] as HTMLAnchorElement | undefined
