...
import type { HeroActionConfig } from '../../shared'
const props = defineProps<{
  config: HeroActionConfig
}>()
const colorClasses = {
  primary: 'btn-primary',
...
const buttonClass = computed(() => {
  const color = props.config.color ?? 'primary'
  const colorClass = props.config?.outline ? colorOutlineClasses[color] : colorClasses[color]
  return [colorClass, { pill: props.config.pill }]
})
</script>
